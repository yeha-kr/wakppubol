...
// 1부: 공 표면 8구역을 탭해 고무 패치로 전부 덮는다 ("팟" 3종 랜덤)
...
const TAP_TOLERANCE = 1.2; // 패치 반지름 대비 탭 인정 배율
...
export default class Stage3 extends Phaser.Scene {
  ...
  create() {
    ...
    // ── 공 그룹: 점토 + 왁스 셸 + 구역 안내선 + (탭 시) 고무 패치 + 케이블 타이 ──
    // 스퀴시 트윈을 한 덩어리로 먹이기 위해 컨테이너로 묶는다
    const clay = this.add.circle(0, 0, CLAY_R, clayColor);
    ...
    // 탭 구역 안내선: 옅은 8분할 윤곽 (텍스트 없이 "여길 탭한다"를 전달)
    this.seamGfx = this.add.graphics();
    for (let i = 0; i < SECTORS; i++) {
      this.seamGfx.lineStyle(2, RUBBER_COLOR, 0.18);
      this.seamGfx.slice(0, 0, PATCH_R, i * this.step, (i + 1) * this.step);
      this.seamGfx.strokePath();
    }

    this.tieGfx = this.add.graphics(); // 2부에서 그린다 (children 순서상 최상단)
    this.ballGroup = this.add.container(BALL_X, BALL_Y, [clay, shell, gloss, this.seamGfx, this.tieGfx]).setDepth(2);
  ...
  onDown(pointer) {
    if (this.phase === 'pat') {
      const idx = this.sectorAt(pointer.x, pointer.y);
      if (idx >= 0 && !this.covered[idx]) this.coverSector(idx);
      return; // 이미 덮인 구역·공 밖은 반응 없음 (명세)
    }
  ...
  sectorAt(x, y) {
    const dx = x - BALL_X;
    const dy = y - BALL_Y;
    if (Math.sqrt(dx * dx + dy * dy) > PATCH_R * TAP_TOLERANCE) return -1;
    const a = Phaser.Math.Angle.Normalize(Math.atan2(dy, dx));
  ...
  coverSector(idx) {
    ...
    // 패치: 부채꼴을 무게중심 원점으로 그려 "팟" 하고 눌러붙는 팝 연출
    ...
    const patch = this.add.graphics({ x: cdx, y: cdy });
    patch.fillStyle(RUBBER_COLOR, 0.92);
    patch.lineStyle(3, RUBBER_EDGE, 0.8);
    patch.slice(-cdx, -cdy, PATCH_R, a0, a0 + this.step);
    patch.fillPath();
    patch.strokePath();
    patch.setScale(0.55).setAlpha(0);
    // 타이보다 아래에 깔리도록 tieGfx 앞에 끼워 넣는다
    this.ballGroup.addAt(patch, this.ballGroup.getIndex(this.tieGfx));
    this.tweens.add({ targets: patch, scale: 1, alpha: 1, duration: 150, ease: 'Back.easeOut' });
